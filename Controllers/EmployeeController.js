...
const index = async (req, res, next) => {
  try {
    ...
    return res
      ...
      .json({
        success: true,
        message: "Employee return Successfully",
        count: data.length,
        data
      });
  } catch (error) {
...
const show = async (req, res, next) => {
  ...
  try {
    ...
    if (!data) {
      return res
        .status(400)
        .json({ success: false, error: "Invalid Employee Detail" });
    }
    return res
      .status(200)
      .json({
        success: true,
        message: "Single Employee return Successfully",
        data
      });
  } catch (error) {
    return res.status(400).json({ success: false, error });
...
const store = async (req, res, next) => {
  ...
  try {
    ...
    return res
      .status(201)
      .json({ success: true, message: "Employee Added Successfully", data });
  } catch (error) {
    return res.status(400).json({ success: false, error });
  }
};
